        ...
        if (args.length == 3) {
            ...
            fs.writeFile("./src/webPanel/admins.json", JSON.stringify(accounts), (err) => {
                if (err) {
                    ...
                    message.channel.send({embeds: [embedHandler.standardEmbed("User Management", "User added successfully!\nAccount Informations are send to the User.", 'RANDOM', message)]});
                    newMember?.send({embeds: [embedHandler.standardEmbed("User Management", "You have been added to the Web Panel!\nName: " + name+ "\nPassword: lwnewuser", 'RANDOM', message)]});
                }
            });
        }
    }
